	...
	return promotionControl ={
			...
			init : function(){
				...
				Event.on('#J_BtnPublish','click dblclick',function(ev){
					var activeState = DOM.val('#J_activeState');
					var typeId = DOM.val('#J_TypeId');
					var editFold = DOM.val('#J_focus');
					var preferential = DOM.val('#J_preferential');
					var unit;
					if(DOM.val('#J_type') == 0){
						unit = '是否将该活动宝贝优惠价修改为 <em class="color-red">'+ editFold + '</em> 元？';
					}else{
						unit = '是否将该活动中宝贝折扣修改为 <em class="color-red">'+ editFold + '</em> 折？';
					}
					if(activeState == 1 && typeId == 2 && editFold != preferential){
						new H.widget.msgBox({
						    title: "编辑活动",
						    content: unit,
						    type: "confirm",
						    buttons: [{ value: "确定修改" }, { value: "取消" }],
	...
	//			}
			...
			checkRule :function(num ,n){
				...
				if(DOM.get('#J_discount_'+num).checked === false && DOM.get('#J_decrease_'+num).checked === false && DOM.get('#J_post_postage_'+num).checked === false && DOM.get('#J_send_gift_'+num).checked === false && IsSend_Point === false ){
					...
					DOM.html('#J_ParamsErrorMsg','层级'+(n+1)+'的优惠内容必须选择一项以上！');
					if (ParamsErrorBox.css("display")==="none") {
						ParamsErrorBox.slideDown();
					}
					return  error=true;
				...
				if(DOM.get('#J_decrease_'+num).checked === true){
					...
					if(error){
						promotionControl.msg.hide();
						if(DOM.val('#J_decrease_money_'+num) == ''){
							//DOM.html('#J_ParamsErrorMsg','价格不能为空哦 <a href="javascript:promotionControl.checkAction('+num+')">点此修改</a>');
							DOM.html('#J_ParamsErrorMsg','价格不能为空哦 ');
						}else{
							//DOM.html('#J_ParamsErrorMsg',result[1]+'<a href="javascript:promotionControl.checkAction('+num+')">点此修改</a>');
							DOM.html('#J_ParamsErrorMsg',result[1]);
						}
					...
					if(con_type == 1){
						...
						if(isOver <=0){
							...
							DOM.html('#J_ParamsErrorMsg','层级'+(n+1)+' 折扣价不能高于条件价');
							if (ParamsErrorBox.css("display")==="none") {
								ParamsErrorBox.slideDown();
							}
							//DOM.val('#J_decrease_money_'+num, '');
				...
				if(con_type == 1){
					if(DOM.get('#J_discount_'+num).checked === true && DOM.get('#J_decrease_'+num).checked === true ){
						...
						if(isOver <=0){
							...
							DOM.html('#J_ParamsErrorMsg','层级'+(n+1)+'满'+con_value+'元打'+discount_rate+'折减'+decrease_money+'元后总价等于'+isOver+'小于0');
							if (ParamsErrorBox.css("display")==="none") {
								ParamsErrorBox.slideDown();
							}
							DOM.val('#J_decrease_money_'+num, '');
					...
					if(DOM.val('#J_is_send_card_'+num) == 1){
						...
						if(card_type != 0){
							...
							if(card_promo_id == 0){
								...
								DOM.html('#J_ParamsErrorMsg','层级'+(n+1)+'请选择卡片名称！');
								if (ParamsErrorBox.css("display")==="none") {
									ParamsErrorBox.slideDown();
								}
								return  error=true;
							...
							DOM.html('#J_ParamsErrorMsg','层级'+(n+1)+'请选择卡片种类!');
							if (ParamsErrorBox.css("display")==="none") {
								ParamsErrorBox.slideDown();
							}
							return  error=true;
