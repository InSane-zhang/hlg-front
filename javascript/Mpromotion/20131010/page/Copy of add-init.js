 ...
 * @author nihao sdfsd
 */
KISSY.add('page/Copy of add-init',function (S,checkUtil,itemHandle,beautifyForm,Select,Tooltip,Calendar,Overlay,XTemplate,showPages,Switchable) {
    var DOM = S.DOM, Event = S.Event;
	return promotionControl ={
			...
			init : function(){
	            ...
	            //满就送弹窗排除宝贝
				Event.on('.J_removeItem','click',function(ev){
					var way = DOM.attr(ev.currentTarget,'way');
					if(way == 'radio'){
						var tex = '选择宝贝';
...
//					}
				...
				Event.on('#J_BtnPublish','click dblclick',function(ev){
					...
					if(DOM.val('#J_TypeId') == 12){
						var promoDesc = DOM.val('#J_promoDesc'),
						startDate = DOM.val('#J_startDate'),
						endDate = DOM.val('#J_endDate'),
						promoFlag = DOM.val('#J_promoName'),
						...
						var str = '<div class="pop-is-promo">'+
								'<div class="ui-about-list" style="margin:0;">'+
									'<ul>'+
										'<li>'+
								            '<div class="ui-side-list">活动名称：</div>'+
								        ...
								        '<li>'+
								            ...
								            '<div class="ui-content-list"><a class="promo-flag">'+ promoFlag +'</a></div> '+                            
								        '</li> '+ 
								        '<li>'+
								           ' <div class="ui-side-list">活动中宝贝：</div>'+
								           ' <div class="ui-content-list"><b class="color-red">'+ instock_num +'</b> 件</div>'+                          
								        '</li> '+ 
								        '<li>'+
		        	...
		        	if(str.length <= 18 && str != ''){
					 ...
					 Event.on('#J_setRegion'+i,'click',function(ev){
						...
						var str = '<div class="pop-area" id="J_areaList">'+
										'<div class="fast-area">'+
											'<a id="J_allArea">全选</a><a id="J_theArea">清空</a><a id="J_hotArea">江浙泸</a>'+
										'</div>'+
				...
				if(promotionControl.isEidt){
		           if(type == 'preview'){
						...
						if(DOM.get('#J_promoExplain').checked == true){
							if(DOM.val(DOM.get("#J_shopDesc")) != '该活动仅限部分用户参与哦'){
