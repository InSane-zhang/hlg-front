KISSY.add('gallery/spotlight/1.0/spotlight',function(S){
	...
	S.augment(Spotlight,S.EventTarget,{
		...
		,nextFocus:function(){
			...
			if(this.quene[index]){
				this.currentIndex = index;
				this.fire(EVENT_NEXT,{target:this.quene[index]});
		...
		,prevFocus:function(){
			...
			if(this.quene[index]){
				this.currentIndex = index;
				this.fire(EVENT_PREVIOUS,{target:this.quene[index]});
				this.focusTo(this.currentIndex,this.config.toggleOnAnim);
			}
		}
		,focusTo:function(index,isAnim){
			...
			this.fire(EVENT_FOCUSTO,{target:node,offset:offset,index:index});
		}
