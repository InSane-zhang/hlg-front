...
//	        DOM.hasClass(body,'has-js') ? '' : DOM.addClass(body,'has-js');
	    ...
	    renderAllCheckPro: function(pid){
	        var self = this;
	        var al = DOM.filter (DOM.query(pid+' input'),function(i){if(i.type =='checkbox' && DOM.hasClass('beautify_input'))return true;});
	        if(al.length < 1){
	    ...
	    toggleRadio: function(id) {
	        ...
	        if (l.className == 'beautify_radio r_off' || i.checked) {
	            ...
	            for (var i = 0; i < ls.length; i++) {
	                var li = ls[i];
	                var lii = DOM.children(ls[i],'input');
	                if (li.className.indexOf('beautify_radio') == -1)  continue;
	                if (DOM.attr(lii,'name') !== iName)  continue;
