			...
			init : function(){
				...
				if(w2Curtime > warnStartTime && w2Curtime < warnEndTime){
					...
					if(S.Cookie.get('hlg-nowarn') != 3){
						new H.widget.msgBox({
						    title:"重要通知",
						    content:'亲，为提高软件性能。今晚 0:30 - 4:00 期间，数据库进行升级，这段时间软件将暂停使用。给您带来不便请见谅哦~<div class="msgInput"><input type="checkbox" id="J_NoticeForNoTitle" name="rememberMe" value="theValue"><text>&nbsp;&nbsp;不在提醒</text></div>',
						    type:"info",
						    buttons: [{ value: "OK" }],
